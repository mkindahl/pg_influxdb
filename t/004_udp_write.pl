...
my $expected = InfluxDB::Test::ResultSet->new(
    '2019-11-26 07:39:14|server01|0.64' );
...
$result = $node->safe_psql( "postgres",
    "SELECT count(*) FROM $schema.disk" );
is( $result, '3', 'multiple data points inserted via single UDP datagram' );

$result = $node->safe_psql( "postgres",
    "SELECT _time, mode, free FROM $schema.disk ORDER BY _time" );
